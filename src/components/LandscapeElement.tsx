...
function LandscapeElement({ filename, reloadChapterData }: { filename: string, reloadChapterData: () => void }) {
  ...
  const handleRemove = () => {
    invoke('remove_landscape', { chapterId, filename })
      .then(() => {
        if (isSelected) { emit('landscapeUnselected'); }
        reloadChapterData()
      })
  ...
  return (
    <>
    <div className="element-item landscape" onContextMenu={handleContextMenu}>
      <img src={`../tableau/assets/landscapes/${filename}`} alt={filename} onClick={handleClick}/>
