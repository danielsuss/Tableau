...
export const defaultCombat:Combat = {
  ...
  mapoffset: {          // 0 for map offset
    x: 0,
    y: 0
  },
  gridsize: 0,          // 0 for gridsize
  gridoffset: {         // 0 for grid offset
    x: 0,
    y: 0
  },
...
export const useReloadChapterData = () => {
  ...
  if (!context) {
    throw new Error('useReloadChapterData must be used within a GlobalStateProvider');
  }
...
export const useUpdateBattlemapId = () => {
  const context = useContext(GlobalStateContext);
  if (!context) {
    throw new Error('useUdateBattlemapId must be used within a GlobalStateProvider')
  }
...
export const useReloadEntityData = () => {
  const context = useContext(GlobalStateContext);
  if (!context) {
    throw new Error('useReloadEntityData must be used within a GlobalStateProvider');
  }
...
export const useOpenDisplayWindow = () => {
  const context = useContext(GlobalStateContext);
  if (!context) {
    throw new Error('useOpenDisplayWindow must be used within a GlobalStateProvider');
  }
...
export const GlobalStateProvider = ({ children }: { children: ReactNode }) => {
  ...
  const [chapterData, setChapterData] = useState<ChapterData>(() => {
    ...
    return storedChapterData ? JSON.parse(storedChapterData) : {
      combat: [],
      landscapes: [],
      splashes: [],
    };
  ...
  const updateBattlemapId = (newBattlemapId: string) => {
  ...
  const reloadChapterData = () => {
    if (chapterId) {
      invoke<ChapterData>('get_chapter_data', { chapterId })
        ...
        .catch((error) => console.error('Failed to reload chapter data:', error));
    }
  };

  // Function to reload entity data from the backend for a specific battlemap
  const reloadEntityData = (battlemapId: string) => {
    if (chapterId) {
      invoke<Entity[]>('get_entities', { chapterId, battlemapId })
        .then((entities) => {
          ...
          localStorage.setItem('entityData', JSON.stringify(entities));
        })
        .catch((error) => console.error('Failed to reload entity data:', error));
    }
  };
  ...
  useEffect(() => {
    const unlistenCombatConstructorDisplay = listen('requestCombatConstructorDisplayData', () => {
      emit('chapterData', chapterData);
      emit('battlemapId', battlemapId);
    });

    const unlistenEntityLocationUpdate = listen('entityLocationUpdate', (event) => {
      const newEntityData = event.payload as Entity;
      invoke('update_entity', { entity: newEntityData })
        .then(() => {
  ...
  return (
    <GlobalStateContext.Provider value={{ chapterId, setChapterId, chapterData, battlemapId, setBattlemapId, updateBattlemapId, entityData, setChapterData, reloadChapterData, reloadEntityData, getCombatData, openDisplayWindow }}>
      {children}
