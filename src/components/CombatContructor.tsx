...
import { Combat, useGlobalState, useOpenDisplayWindow, useReloadChapterData, useReloadEntityData, useUpdateBattlemapId } from "./GlobalStateContext"; // Import the global state
...
function CombatConstructor () {
    const { chapterData, chapterId, entityData, battlemapId } = useGlobalState(); // Access chapterId from the global context
    ...
    const handleShowDisplay = () => {
        invoke('generate_hexgrid', {containerWidth: 1667, containerHeight: 953, hexSize: combatData.gridsize, overflow: 3, outputPath: `../tableau/assets/hexgrids/${combatData.battlemap}`})
            .then(()=> {
                reloadChapterData();
    ...
    return (
        <div className="constructor-container">
            ...
            <div className="nav-bar">
                ...
                <div className="header-container">Combat Constructor: Chapter {chapterId}</div>
            ...
            <div className="entities-container">
                ...
                <div className="icons">
                    {entityData.map((entity, index) => (
                        <div key={index} className={`entity-container-clicked-${selectedEntity === entity.icon}`} onClick={() => handleEntityClick(entity.icon)}>
                ...
                <div className="rightside-container">
                    ...
                    <div className="editor-properties-buttons">
                        <button className="editor-button" onClick={handleEditorButton} disabled={editorProperties === 'editor'}>Editor</button>
                        <button className="properties-button" onClick={handlePropertiesButton} disabled={editorProperties === 'properties'}>Properties</button>
                    </div>
                    ...
                    <div className="editor-properties-container">
                        ...
                        {editorProperties === 'editor' ? (
                            ...
                            selectedEntity === "" ? (
                                <div className="icon-properties-placeholder">Select entity to view properties</div>
                            ) : (
                                <PropertiesEditor entity={entityData.find(entity => entity.icon === selectedEntity)!} battlemap={battlemap} setEditorProperties={setEditorProperties} />
                            )
                        )}                                          
                    </div>                    
                </div>  
